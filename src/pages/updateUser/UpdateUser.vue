...
export default {
  ...
  methods: {
    async getDataFromUser() {
      const respone = await UserService.getUserData(this.$route.params.userId, {
        userId: this.userData?.userId,
      });
      this.user.userId = respone.data?.userId;
      this.user.email = respone.data?.email.split("@")[0];
      this.user.domain = respone.data?.email.split("@")[1];
      this.user.name = respone.data?.name;
      this.user.mobilePhone = respone.data?.mobilePhone;
      this.user.role = respone.data?.role;
      this.user.avatar =
        "http://localhost:8081/avatar/images/" + respone.data?.avatar;
    },
    async updateData() {
      try {
